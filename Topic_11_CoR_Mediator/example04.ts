...
class AccessControlSystem {
  ...
  processRequest(request: AccessRequest): AccessResult {
    // Журналювання запиту
    console.log(
      `[${new Date().toISOString()}] [ACCESS_CONTROL] Обробка запиту: ${request.role} -> ${request.resource}`
    );
    ...
    console.log(
      `[${new Date().toISOString()}] [ACCESS_CONTROL] Доступ заборонено за замовчуванням: ${request.role} -> ${request.resource}`
    );
...
class AdminAccessRule implements AccessRule {
  ...
  handle(request: AccessRequest): AccessResult {
    console.log(
      `[${new Date().toISOString()}] [ADMIN_RULE] Доступ до ресурсу дозволено адміністратору ${request.resource}`
    );
...
class BusinessHoursRule implements AccessRule {
  ...
  handle(request: AccessRequest): AccessResult {
    console.log(
      `[${new Date().toISOString()}] [BUSINESS_HOURS_RULE] Доступ до ресурсу дозволено користувачу в робочий час  ${request.resource}`
    );
    return AccessResult.GRANTED;
  }
}
...
class GuestAccessRule implements AccessRule {
  ...
  handle(request: AccessRequest): AccessResult {
    console.log(
      `[${new Date().toISOString()}] [GUEST_RULE] Гостю надано обмежений доступ до ресурсу ${request.resource}`
    );
...
class SensitiveResourceRule implements AccessRule {
  ...
  handle(request: AccessRequest): AccessResult {
    console.log(
      `[${new Date().toISOString()}] [SENSITIVE_RULE] Доступ до захищеного ресурсу заборонено  ${request.resource}`
    );
    return AccessResult.DENIED;
  }
}
